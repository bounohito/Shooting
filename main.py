...
TheGame, f_assets, f_Images, f_made, f_Sounds = '' ,'assets/', 'Images/', 'made/', 'Sounds/'
...
# Player
playerImg = py.image.load(f'{TheGame}{f_assets}{f_Images}stand.png')

# Huge Ball
huge_ballImg = py.image.load(f'{TheGame}{f_assets}{f_Images}huge_ball.png')
...
for num in JarsType:
    img = Image.open(f'{TheGame}{f_assets}{f_Images}{num}.png')
    img_resized = img.resize((95, 100))
    img_resized.save\
        (f'{TheGame}{f_assets}{f_Images}{f_made}{num}_resized.png', quality = 90)

    img_mirror = ImageOps.mirror(img_resized)
    img_mirror.save\
        (f'{TheGame}{f_assets}{f_Images}{f_made}{num}_mirror.png', quality = 90)

    JarImg.append(py.image.load\
                  (f'{TheGame}{f_assets}{f_Images}{f_made}{num}_resized.png'))

    JarImg_mirror.append(py.image.load\
                         (f'{TheGame}{f_assets}{f_Images}{f_made}{num}_mirror.png'))
...
# Sounds
Dun_Dun_Dunnn = mixer.Sound(f'{TheGame}{f_assets}{f_Sounds}Dun_Dun_Dunnn.ogg')
meow = mixer.Sound(f'{TheGame}{f_assets}{f_Sounds}meow.ogg')
pop = mixer.Sound(f'{TheGame}{f_assets}{f_Sounds}pop.ogg')
...
async def main():
    ...
    # Variables
    global running, playerX, playerY, huge_ballX, huge_ballY,\
        huge_ball_state, huge_ballX_change, huge_ballY_change, fever, Jarnum, obtain,\
            all_clean_time, all_clean_type, game_over, t, lastX_change, lastY_change, num
    ...
    while running:
        ...
        if game_over:
            ...
            Sound_stop_and_play(Dun_Dun_Dunnn)
            py.display.flip()
            running = True
            t = 0
